...
Next we will consider a modified version of Kinship2 sample pedigree, where it 
contains the founder which is both affected and has DNA available. 
```{r}
# load pedigree data from Kinship2  
data("sample.ped") #two pedigrees
testPed1=sample.ped[sample.ped$ped==2,] #2nd pedigree
testPed1[testPed1$id==203,"affected"]=0
testPed1[testPed1$id==211,"affected"]=1
testPed1[testPed1$id==202,"affected"]=0
testPed1[testPed1$id==201,"avail"]=1
testPed1[testPed1$id==203,"avail"]=0
testPed1[testPed1$id==204,"avail"]=0
testPed1[testPed1$id==212,"avail"]=0
testPed1 # Unknown affection status has NA
```

From the above data-frame, we see that founder 201 is affected with DNA available. 
Based on our previous assumption that RV enters the pedigree through a single 
founder only, thus the events that exactly one founder introduced the RV are mutually
exclusive. And the assumption will be violated when other founders introduced the
RV except ID 201, as well as some configuration settings. 

For example, if the RV configuration is C=(1,1,1,1,1), we assumed only the event
that ID 201 introduced the RV will be calculated, the conditional probabilities 
of the configuration given exactly one of the other founders introduced RV will 
be calculated as zero. Additionally, if the RV configuration is C=(0,1,1,1,1), 
which conflicts with the event that ID 201 introduced the RV, we assumed this conditional 
probability will be calculated as zero, and the conditional probabilities of 
configuration given ID 201 or ID 203, or ID 209 will be calculated as usual. 
```{r}
# config = c("1", "1", "1", "1", "1")
# names(config)=c("201","206","207","211", "214")
# likehd(testPed1, 1/2, config)
# 
# config = c("0", "1", "1", "1", "1")
# names(config)=c("201","206","207","211", "214")
# likehd(testPed1, 1/2, config)
...
However, when we ran the above code, it gives error, we found that when a founder 
is affected with DNA available, the querygrain() function we used to calculate 
the marginal distribution of RV status for ID 201 is Null. In this project, we 
only consider pedigrees with founders either unaffected or affected with DNA unavailable. 
